...
goto(`${BASE_PATH}/courses/getting-started/lesson/${nextPrevLessons.next.id}`)
}
...
goto(`${BASE_PATH}/courses/getting-started/lesson/${nextPrevLessons.prev.id}`)
}
...
<div class="min-h-screen bg-white" style="margin:0 auto;">
  ...
  <div class="container mx-auto px-4 max-w-6xl py-6">
    ...
    margin-top: 10px;
"
            href="/home/courses/getting-started/"
            class="px-4 text-sm font-medium text-white {isLoading ? 'bg-gray-400' : 'bg-blue-500'} rounded-md {isLoading ? 'hover:bg-gray-400' : 'hover:bg-blue-600'} transition-all duration-200 flex items-center whitespace-nowrap cursor-pointer active:scale-95 active:bg-blue-700 transform"
            title="Go to Chapters"
          >
